...
{
	/// <summary>
	/// Implementation of the Unit of Work pattern to process an <see cref="Action"/> with non-transactional operations on domain entities.
	/// </summary>
	public sealed class NonTransactionalUnitOfWork
		: UnitOfWork
	{
		#region - Constructors - 
        ...
        public override T Start<T>(Func<T> commit, Func<T> rollback, IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            ...
            try
            {
                ...
                using (TransactionScope scope = new TransactionScope(DomainConstant.TRANSACTION_SCOPE_OPTION_SUPPRESS, options))
                {
